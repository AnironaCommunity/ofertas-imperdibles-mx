...
async function drawShareSummaryImage(selectedCoupons, link, totalActive) {
  ...
  const columns = selectedCoupons.length <= 2 ? 2 : selectedCoupons.length <= 6 ? 3 : 5;
  const columnGap = 16;
  const rowGap = 18;
  ...
  const badgeY = outerPadding + 20;
  context.save();
  context.shadowColor = "rgba(15, 23, 42, .10)";
  context.shadowBlur = 14;
  context.shadowOffsetY = 4;
  roundedRect(context, badgeX, badgeY, badgeWidth, 72, 16);
  context.fillStyle = "#ffffff";
  context.fill();
  context.restore();
  context.fillStyle = "#16a34a";
  context.beginPath();
  context.arc(badgeX + 34, badgeY + 36, 19, 0, Math.PI * 2);
  context.fill();
  context.fillStyle = "#ffffff";
  ...
  const bankPalette = {
    bbva: { accent: "#1464d2", soft: "#eef5ff" },
    banamex: { accent: "#1464d2", soft: "#eef5ff" },
    santander: { accent: "#e5242a", soft: "#fff1f1" },
    hsbc: { accent: "#e5242a", soft: "#fff1f1" },
    "american-express": { accent: "#1464d2", soft: "#eef5ff" },
    afirme: { accent: "#159447", soft: "#effaf3" },
    inbursa: { accent: "#173b7a", soft: "#f1f5fb" },
    banorte: { accent: "#e5242a", soft: "#fff1f1" },
    scotiabank: { accent: "#e5242a", soft: "#fff1f1" },
    openbank: { accent: "#5b2a86", soft: "#f7f1fb" },
  ...
  function normalizeDiscount(coupon) {
    ...
    const numericOff = raw.match(/(?:^|\s)([\d,.]+)\s*OFF\b/i);
    if (numericOff) return numericOff[1];
    return raw.replace(/\boff\b/i, "").trim();
  }

  function fittedText(text, x, y, maxWidth, preferredSize, minimumSize, weight = 700) {
  ...
  selectedCoupons.forEach((coupon, index) => {
    ...
    context.fillRect(x, y, cardWidth, 54);
    context.fillStyle = palette.accent;
    context.fillRect(x, y + cardHeight - 4, cardWidth, 4);
    context.restore();

    // Encabezado de tarjeta: logo bancario si existe; Tienda/Exclusivo usa solo texto.
    if (isBank && cardAssets[index]) {
      drawImageContain(context, cardAssets[index], x + 24, y + 11, cardWidth - 48, 32);
    } else {
      context.fillStyle = palette.accent;
      context.textAlign = "center";
    ...
    const infoY = y + 174;
    context.fillStyle = "#f8fafc";
    context.fillRect(x, infoY, cardWidth, cardHeight - (infoY - y));
    context.strokeStyle = "#e2e8f0";
    context.lineWidth = 1;
    context.beginPath();
    context.moveTo(x + cardWidth / 2, infoY + 14);
    context.lineTo(x + cardWidth / 2, y + cardHeight - 16);
    context.stroke();
    ...
    context.fillStyle = "#111827";
    fittedText(String(coupon.compra_minima || "Consultar"), x + cardWidth * 0.25, infoY + 49, cardWidth * 0.42, 17, 12, 800);
  ...
  return await new Promise((resolve, reject) => {
    canvas.toBlob((blob) => {
      ...
      else reject(new Error("No fue posible generar la imagen."));
    }, "image/png", 0.95);
  });
}
...
function printCouponCardHtml(coupon, activeList) {
  ...
  const soldOut = coupon?.agotado === true;
  ...
  return `<article class="cupon cupon-horizontal-v16${exclusive ? " cupon-exclusivo" : ""}${soldOut ? " cupon-agotado" : ""}" style="--categoria-cupon-color:${color};--categoria-cupon-texto:#fff;--ticket-cutout-bg:#fff"><span class="ticket-notch ticket-notch-top"></span><span class="ticket-notch ticket-notch-bottom"></span>${watermark}<div class="hc16-valor">${image}<h2 class="hc16-descuento descuento">${title}<span class="hc19-off">OFF</span></h2><span class="hc19-porcentaje">%</span></div><div class="hc16-info"><div class="hc16-categoria">${soldOut ? "CUPÓN AGOTADO" : categoryText}</div><div class="hc16-condiciones"><p class="hc16-condicion">En compras desde <strong>${printEscape(coupon?.compra_minima || "Consultar")}</strong></p></div>${detail ? `<p class="hc16-detalle">${printEscape(detail)}</p>` : ""}${percent ? `<p class="hc16-ahorro-extra">Ahorra hasta <strong>${printEscape(coupon?.ahorro_maximo || "Consultar")}</strong></p>` : ""}<div class="hc16-etiquetas">${tags}</div></div><div class="hc16-acciones"><div class="hc16-cta acciones-cupon"><button class="boton-canjear hc16-copiar" type="button">${copyButton}</button></div><p class="mensaje hc16-mensaje"></p>${social}<div class="estado-programacion hc16-tiempo">${printEscape(remaining)}</div></div></article>`;
}
...
function actualizarSelectorBanco() {
  ...
  if (couponBankDetail) {
    couponBankDetail.placeholder = esBancario
      ? "Captura el detalle del beneficio bancario"
      : esTienda
        ? "Ejemplo: Aplica en productos participantes"
        : "Ejemplo: Exclusivo para productos seleccionados";
  }
...
couponCategory?.addEventListener("change", actualizarSelectorBanco);
couponBank?.addEventListener("change", () => {
  if (!couponBank.value) return;
  couponImage.value = "";
  couponImageUrl.value = couponBank.value;
  couponImagePreview.src = couponBank.value;
  couponImagePreviewWrapper.hidden = false;
