...
function setMessage(element, text = "", isError = false) {
  ...
  if (typeof text === "string") {
    ...
    message = text.message || "No fue posible completar la operación.";
  } else {
...
async function saveHeroConfig(event) {
  ...
  try {
    ...
    const savedConfig = await api("/api/admin-hero-config?v=67.15.4", {
      method: "PUT",
      body: JSON.stringify({
        imagen_url: imageUrl || "",
    ...
    if (savedConfig.api_version !== "67.15.4") {
      throw new Error(
        "Vercel todavía está usando una versión anterior de /api/admin-hero-config."
      );
    }
